...
export default async function ComparePage({
  ...
  const getVal = (country: any, kind: string, label: string) =>
    country?.metrics?.find((x: any) => x.kind === kind && x.label === label)?.value;
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="flex gap-3 card p-4">
        <input
          name="a"
          placeholder="страна-a (slug)"
          className="border p-2 rounded-lg"
          defaultValue={searchParams.a ?? ""}
        />
        <input
          name="b"
          placeholder="страна-b (slug)"
          className="border p-2 rounded-lg"
      ...
      <p className="text-xs text-gray-500">
        Пример: <code>?a=germany&b=canada</code>
      </p>
    </div>
  );
}
